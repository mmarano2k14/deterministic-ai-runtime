...
namespace Multiplexed.AI.Runtime.Publication
{
    public sealed partial class AiImmutablePublicationStore
    {
        ...
        private async Task<AiWorkerCodeBundle> ReadPolicyWorkerCodeCoreAsync(string executionId,string policyName,string scope,string? ownerStepName,string language,string implementationRef,AiPublicationFunctionKind kind,AiPublicationIdentity.Guard guard,CancellationToken token)
        {
            ...
            AiDependencyPackagingContracts.RequireExecutionSupported(environment.Dependencies,language);
            ...
            var sources=await Files(implementation.Sources).ConfigureAwait(false); var dependencies=new List<AiWorkerDependency>(environment.Dependencies.Count); foreach(var dependency in environment.Dependencies) dependencies.Add(new AiWorkerDependency(dependency.Name,dependency.Version,await Files(dependency.Files).ConfigureAwait(false)) { Package = dependency.Package });
