...
namespace Multiplexed.AI.Runtime.Invocation
{
    ...
    {
        private readonly IReadOnlyDictionary<string,IAiRetryPolicyTransport> _transports; private readonly TimeSpan _timeout;
        public AiRetryPolicyAdapterFactory(IEnumerable<IAiRetryPolicyTransport> transports, AiRetryPolicyInvocationOptions? options=null)
        {
            ArgumentNullException.ThrowIfNull(transports); _timeout=(options??new()).EvaluationTimeout;
            ...
            foreach(var transport in transports)
            {
                ...
                if (transport.ExecutionLanguage is not (AiExecutionLanguages.DotNet or AiExecutionLanguages.Python or AiExecutionLanguages.TypeScript))
        ...
        public IAiPolicy Bind(AiStepExecutionContext context, AiConfiguredPolicyDefinition declaration, AiPolicyInvocationBinding binding)
        {
            ...
            return new AiRetryPolicyAdapter(transport,binding,declaration,tenant,group,context.ExecutionId,pipelineKey,context.StepName,context.StepKey,context.CancellationToken,_timeout);
        }
    }
}
