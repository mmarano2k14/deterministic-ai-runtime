...
namespace Multiplexed.AI.Runtime.Invocation
{
    ...
    public sealed class AiRetryPolicyAdapter : IAiPolicy, IAiPolicyInvocationIdentity
    {
        ...
        private readonly CancellationToken _executionCancellation; private readonly TimeSpan _timeout; private readonly TimeProvider _timeProvider;
        ...
        public AiRetryPolicyAdapter(IAiRetryPolicyTransport transport, AiPolicyInvocationBinding binding, AiConfiguredPolicyDefinition declaration,
            ...
            CancellationToken executionCancellation, TimeSpan timeout, TimeProvider? timeProvider=null)
        {
            _transport=transport; _binding=binding; _declaration=declaration; _tenantId=tenantId; _tenantGroupId=tenantGroupId;
            _executionId=executionId; _pipelineKey=pipelineKey; _stepName=stepName; _stepKey=stepKey; _executionCancellation=executionCancellation; _timeout=timeout; _timeProvider=timeProvider??TimeProvider.System;
        ...
        public async Task<AiPolicyResult> ExecuteAsync(object context, CancellationToken cancellationToken=default)
        {
            ...
            var request = new AiRetryPolicyRequest(_requestId, Key, _binding.Scope.ToString(), _binding.OwnerStepName,
                _binding.Invocation.ExecutionLanguage!, _binding.Invocation.ImplementationRef!, _timeProvider.GetUtcNow().Add(_timeout),
