...
namespace Multiplexed.AI.Tests.Runtime.Invocation.Workers.Policies
{
    ...
    public sealed class AiHostedRetryPolicyTransportTests
    {
        [Theory]
        [InlineData("python")]
        ...
        {
            Assert.ThrowsAny<ArgumentException>(() => new AiHostedRetryPolicyTransport(
        ...
        public async Task Evaluation_Uses_The_Exact_Policy_Request_As_Portable_Inputs()
        {
            ...
            Assert.Equal("retry", response.GetProperty("decision").GetString());
            var wire = Assert.Single(fixture.Transport.Requests);
            Assert.Equal(fixture.Request.RequestId, wire.RequestId);
            Assert.Equal("retry-policy-" + fixture.Request.RequestId, wire.OperationId);
            ...
            Assert.Equal("retry", wire.Inputs.GetProperty("policyKind").GetString());
            Assert.Equal("retry", response.GetProperty("decision").GetString());
        }
        ...
        private sealed class Fixture
        {
            ...
            internal readonly AiHostedRetryPolicyTransport Policy;
            internal readonly AiRetryPolicyRequest Request;

            internal Fixture(string language = "python")
            {
                Preparer = new Preparer(language);
                Policy = new AiHostedRetryPolicyTransport(language, Preparer, Transport);
        ...
        private sealed class Preparer : IAiRetryPolicyCodePreparer
        {
            internal int Calls;
            ...
            public Task<AiWorkerCodeBundle> PrepareAsync(AiRetryPolicyRequest request, CancellationToken cancellationToken = default)
            {
        ...
        private static AiRetryPolicyRequest CreateRequest(string language, string implementationRef) => new(
            "request-policy-a",
            "retry-guard",
            "Pipeline",
            null,
