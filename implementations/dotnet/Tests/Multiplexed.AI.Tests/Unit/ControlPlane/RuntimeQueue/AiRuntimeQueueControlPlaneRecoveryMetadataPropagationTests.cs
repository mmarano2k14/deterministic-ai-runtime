...
namespace Multiplexed.AI.Tests.Unit.ControlPlane.RuntimeQueue
{
    ...
    public sealed class AiRuntimeQueueControlPlaneRecoveryMetadataPropagationTests
    {
        ...
        private sealed class FakeRuntimePipelineBackgroundController : IAiRuntimePipelineBackgroundController
        {
            ...
            /// <inheritdoc />
            public ValueTask<AiRuntimeWorkerRunHandle> EnqueueResumeAsync(AiRuntimePipelineRunRequest request, string executionId, CancellationToken cancellationToken = default)
            {
                EnqueueResumeCalls++;
            ...
            /// <inheritdoc />
            public Task<bool> CancelRunAsync(AiRuntimeWorkerRunHandle handle, string? reason = null, string? requestedBy = null, CancellationToken cancellationToken = default) => Task.FromResult(false);
