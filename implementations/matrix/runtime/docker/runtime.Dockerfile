...
RUN dotnet publish implementations/matrix/fixtures/mcp-effect-probe/Multiplexed.AI.Matrix.McpEffectProbe/Multiplexed.AI.Matrix.McpEffectProbe.csproj -c Release --nologo --verbosity quiet -p:PublishDir=/out/mcp-effect-probe/
...
COPY --from=dotnet-build /out/mcp-effect-probe /app/mcp-effect-probe
...
RUN chmod 0555 /app/runtime-entrypoint.sh \
 && chmod -R a-w /app/runtime /app/workers /app/mcp-effect-probe \
