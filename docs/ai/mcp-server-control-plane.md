...
See [Hosted Multilanguage Execution](hosted-multilanguage-execution.md#outbound-mcp-and-effect-identity) for the transport contract, [Durable MCP Effect Evidence](durable-mcp-effect-evidence.md) for the durable state/reconciliation boundary, and [Durable MCP Effect Evidence Validation](durable-mcp-effect-evidence-validation.md) for its evidence. A separate public SDK server adapter now exposes `sdk.publish_pipeline` and execution submit/observe/result/cancel operations through the MCP server; those tools map into existing runtime authorities and are not the outbound business-effect transport described here. None of these boundaries claim generic exactly-once provider behavior.
