...
![AI Runtime Architecture](images/ai-runtime/ai-runtime-architecture.png)
...
![Distributed Concurrency & Throttling Architecture](images/ai-runtime/ai-runtime-concurrency-engine.png)
...
![AI Step Retry Engine Architecture](images/ai-runtime/ai-runtime-retry-engine.png)
...
![AI Execution State Retention](images/ai-runtime/ai-runtime-retention-architecture.png)
...
### Hosted Multilanguage Execution Reference
...
The runtime includes a **validated durable Child DAG composition** primitive that allows one durable execution to delegate to another DAG, park through `WaitingForExternal`, recover nested work through the existing recovery path, and resume through deterministic continuation. Recursive production validation now reaches `ChildDepth = 3`, with EventDriven lifecycle observation and the existing Ledger, Runtime Lifecycle Journal, replay, trace, and Recovery Forensics providing correlated proof.
...
The runtime also has a **validated durable Child DAG composition** foundation for nested execution and future multi-agent orchestration. It reuses the same deterministic execution, queue, recovery, canonical event observation, Ledger, Runtime Lifecycle Journal, replay, trace, and Forensics boundaries. Recursive validation is green through Depth3, the bounded recursive Depth3 production proof includes exact per-depth child logical-step Ledger accounting, and the selected deterministic multi-interleaving adversarial matrix is green across HTTP/gRPC × ProcessHostPool/KubernetesPool. Dedicated recursive-child replay and exhaustive state-space exploration remain distinct future proof domains.
