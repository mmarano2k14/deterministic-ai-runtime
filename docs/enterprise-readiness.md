...
| SDK availability | Portable public SDK contracts, the authorized publication/execution server boundary, and independent .NET, TypeScript/JavaScript, and Python SDK client packages are implemented and validated. The fixture-free Docker ProcessHostPool runtime matrix is GREEN at 33/33 scenarios. Public registry publication and the standalone CLI remain separate productization work. |
