...
| Can one durable DAG delegate to another DAG and resume safely? | Yes, through native durable Child DAG composition. The parent parks in `WaitingForExternal`, child identity and input are durable, completion precedes continuation, and the same parent `ExecutionId` resumes. The capability is still labeled **Experimental** until full engine lifecycle observation is aligned across Lifecycle Events, durable Ledger, and Forensics and deeper nested closure is complete. | Existing DAG engine, Child DAG relation/snapshot state, Policy Engine, shared queue, runtime recovery, replay, Ledger, tracing, lifecycle evidence, and Forensics. | Full `ChildDepth = 1` gRPC Kubernetes Runtime Pool warm-reuse proof is green; focused deeper-nesting proofs exist, while the complete bounded Depth 2 closure remains pending. | **Experimental / validated at current Depth 1 boundary** |
