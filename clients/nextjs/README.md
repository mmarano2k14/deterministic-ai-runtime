...
Sample/backend/runtime integration:
...
AI features use the provider configured by the sample API.
...
dotnet run \
  --project Samples/MultiplexedRbac.Sample.Crm.Api/MultiplexedRbac.Sample.Crm.Api.csproj
```

The sample API exposes the demo endpoints used by the Next.js client, including login, scenario execution, runtime analysis, approval, verification, and realtime events.
