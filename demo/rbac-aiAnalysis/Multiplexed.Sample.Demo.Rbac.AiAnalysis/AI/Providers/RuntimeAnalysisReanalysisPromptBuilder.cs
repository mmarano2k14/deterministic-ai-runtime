...
namespace Multiplexed.Sample.Demo.Rbac.AiAnalysis.AI.Providers
{
    internal static class RuntimeAnalysisReanalysisPromptBuilder
    {
        public const string Instructions =
            ...
            - The original request may contain scenarioPolicyDefinition. When present, it is the exact deterministic policy envelope used by the same runtime-analysis DAG.
            - Treat scenarioPolicyDefinition as a HARD proposal-generation boundary: never propose a follow-up that would be denied by those configured policies.
            - Use only allowed scenario types, keep all numeric values inside configured limits, and choose MaxInFlight only from allowedMaxInFlightValues.
            ...
            - Policy awareness does not bypass the runtime policy step or human approval.
            ...
            RuntimeAnalysisReanalysisProviderRequest request)
        {
            ...
            var payload = new
            {
                ...
                scenarioPolicyDefinition =
                    request.OriginalRequest.ScenarioPolicyDefinition
            };
